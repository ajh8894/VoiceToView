...
package com.swmem.voicetoview.temp;
...
import com.swmem.voicetoview.jni.FLACStreamEncoder;
...
public class FLACRecorder extends Thread {
	...
	private long getRms(int end, int span) {
		...
		for (int i = begin; i < end; i += 2) {
			// TODO: We don't need the whole short, just take the 2nd byte (the
			// more significant one)
			// byte curSample = mCurrentRecording[i+1];

			short curSample = getShort(buffer.get(i), buffer.get(i + 1));
	...
	public double getPauseScore() {
		long t2 = getRms(mRecordedLength, mBytesPerSecond);
		if (t2 == 0) {
			return 0;
		}
	...
	public void run() {
		...
		final int configs[] = { AudioFormat.CHANNEL_IN_MONO,
			...
			for (int y = 0; !found && y < sample_rates.length; ++y) {
				...
				for (int z = 0; !found && z < configs.length; ++z) {
					...
					try {
						...
						if (istate != AudioRecord.STATE_INITIALIZED) // it lied
																		// to us
							continue;
		...
		Log.d(LTAG, "Using: " + format + "/" + channel_config + "/"
				+ sample_rate);
		...
		try {
			...
			mBytesPerSecond = sample_rate * (mapped_format / 8) * mapped_channels;
				...
				if (mShouldRecord) {
					...
					default:
						if (result > 0) {
							...
							// s.rewind();
							// for (int i = 0 ; i < (result / (format / 8)) ;
							// ++i) {
							// short v = s.get(i);
							// if (Math.abs(v) <= 255) {
							// Log.d(LTAG, "*** CHIPMUNK sample " + i + ": " +
							// v);
							// }
							// }

							// Compute time recorded
							double read_ms = (1000.0 * result)
									/ mBytesPerSecond;
							mDuration += read_ms;
							...
							// Log.d(LTAG, "Write of " + result + " bytes took "
							// + (end - start) + " msec.");
						}
					}
				}
