 ...
 * Constants
 **/
static char const * const FLACStreamDecoder_classname =
		"com.swmem.voicetoview.jni.FLACStreamDecoder";
...
FLAC__StreamDecoderReadStatus flac_read_helper(
		FLAC__StreamDecoder const * decoder, FLAC__byte buffer[],
		size_t * bytes, void * client_data);

FLAC__StreamDecoderSeekStatus flac_seek_helper(
		FLAC__StreamDecoder const * decoder, FLAC__uint64 absolute_byte_offset,
		void * client_data);
...
FLAC__StreamDecoderLengthStatus flac_length_helper(
		FLAC__StreamDecoder const * decoder, FLAC__uint64 * stream_length,
		void * client_data);

FLAC__bool flac_eof_helper(FLAC__StreamDecoder const * decoder,
		void * client_data);

FLAC__StreamDecoderWriteStatus flac_write_helper(
		FLAC__StreamDecoder const * decoder, FLAC__Frame const * frame,
		FLAC__int32 const * const buffer[], void * client_data);

void flac_metadata_helper(FLAC__StreamDecoder const * decoder,
		FLAC__StreamMetadata const * metadata, void * client_data);

void flac_error_helper(FLAC__StreamDecoder const * decoder,
...
public:
	...
	FLACStreamDecoder(char * infile) :
			m_infile_name(infile), m_infile(NULL), m_sample_rate(-1), m_total_samples(
					-1), m_channels(-1), m_bits_per_sample(-1), m_min_buffer_size(
					-1), m_decoder(NULL), m_finished(false), m_seek_pos(-1), m_cur_pos(
					-1), m_buffer(NULL), m_buf_size(-1), m_buf_used(-1) {
	}
	...
	char const * const init() {
		...
		aj::log(ANDROID_LOG_DEBUG, LTAG, "FLAC__StreamDecoder opened %s",
				m_infile_name);

		// Try initializing the file stream.
		FLAC__StreamDecoderInitStatus init_status =
				FLAC__stream_decoder_init_stream(m_decoder, flac_read_helper,
						flac_seek_helper, flac_tell_helper, flac_length_helper,
						flac_eof_helper, flac_write_helper,
		...
		FLAC__bool result = FLAC__stream_decoder_process_until_end_of_metadata(
				m_decoder);
		if (!result) {
			return "Could not read metadata from FLAC__StreamDecoder!";
		}
		aj::log(ANDROID_LOG_DEBUG, LTAG,
				"FLAC__StreamDecoder read metadata OK");
	...
	int read(char * buffer, int bufsize) {
		...
		do {
			if (m_seek_pos >= 0) {
				...
				result = FLAC__stream_decoder_seek_absolute(m_decoder,
						m_seek_pos);
			...
			ret = checkState();
			aj::log(ANDROID_LOG_DEBUG, LTAG,
					"result: %d, used: %d, size: %d, ret: %d", result,
					m_buf_used, m_buf_size, ret);
	 ...
	 **/
	FLAC__StreamDecoderReadStatus cb_read(FLAC__StreamDecoder const * decoder,
	...
	FLAC__StreamDecoderSeekStatus cb_seek(FLAC__StreamDecoder const * decoder,
			FLAC__uint64 absolute_byte_offset) {
		if (0
				> fseeko(m_infile, static_cast<off_t>(absolute_byte_offset),
						SEEK_SET)) {
	...
	FLAC__StreamDecoderTellStatus cb_tell(FLAC__StreamDecoder const * decoder,
			FLAC__uint64 * absolute_byte_offset) {
		off_t pos = 0;
	...
	FLAC__bool cb_eof(FLAC__StreamDecoder const * decoder) {
		if (feof(m_infile)) {
			m_finished = true;
		}
		return m_finished;
	...
	FLAC__StreamDecoderWriteStatus cb_write(FLAC__StreamDecoder const * decoder,
	...
	void cb_metadata(FLAC__StreamDecoder const * decoder,
		...
		m_min_buffer_size = metadata->data.stream_info.max_blocksize
				* (m_bits_per_sample / 8) * m_channels;
	}

	void cb_error(FLAC__StreamDecoder const * decoder,
...
private:
	...
	FLAC__StreamDecoderWriteStatus write_internal(int blocksize,
...
static FLACStreamDecoder * get_decoder(JNIEnv * env, jobject obj) {
	...
	jfieldID object_field = env->GetFieldID(cls, FLACStreamDecoder_mObject,
			"J");
 ...
 **/
static void set_decoder(JNIEnv * env, jobject obj,
...
void Java_com_swmem_voicetoview_jni_FLACStreamDecoder_init(JNIEnv * env, jobject obj,
		jstring infile) {
	assert(sizeof(jlong) >= sizeof(FLACStreamDecoder *));

	aj::log(ANDROID_LOG_DEBUG, LTAG, "FLACStreamDecoder_init.., infile=%s",
...
void Java_com_swmem_voicetoview_jni_FLACStreamDecoder_deinit(JNIEnv * env, jobject obj) {
	FLACStreamDecoder * decoder = get_decoder(env, obj);
	delete decoder;
	set_decoder(env, obj, NULL);
}

jint Java_com_swmem_voicetoview_jni_FLACStreamDecoder_read(JNIEnv * env, jobject obj,
...
jint Java_com_swmem_voicetoview_jni_FLACStreamDecoder_bitsPerSample(JNIEnv * env,
		jobject obj) {
	FLACStreamDecoder * decoder = get_decoder(env, obj);
...
jint Java_com_swmem_voicetoview_jni_FLACStreamDecoder_channels(JNIEnv * env,
		jobject obj) {
	FLACStreamDecoder * decoder = get_decoder(env, obj);
...
jint Java_com_swmem_voicetoview_jni_FLACStreamDecoder_sampleRate(JNIEnv * env,
		jobject obj) {
	FLACStreamDecoder * decoder = get_decoder(env, obj);
...
jint Java_com_swmem_voicetoview_jni_FLACStreamDecoder_minBufferSize(JNIEnv * env,
		jobject obj) {
	FLACStreamDecoder * decoder = get_decoder(env, obj);
...
jint Java_com_swmem_voicetoview_jni_FLACStreamDecoder_totalSamples(JNIEnv * env,
		jobject obj) {
	FLACStreamDecoder * decoder = get_decoder(env, obj);
...
void Java_com_swmem_voicetoview_jni_FLACStreamDecoder_seekTo(JNIEnv * env, jobject obj,
		jint sample) {
	FLACStreamDecoder * decoder = get_decoder(env, obj);
...
jint Java_com_swmem_voicetoview_jni_FLACStreamDecoder_position(JNIEnv * env,
		jobject obj) {
	FLACStreamDecoder * decoder = get_decoder(env, obj);
